...
const (
	// testProvider is "capmox" (not "proxmox") because pkg/image.GetImageKey only
	// derives a .qcow2 S3 key for the capmox provider value - which is what the
	// release controller sets for Proxmox releases. Using it keeps the seeded key
	// and the reconciler's lookup on the realistic qcow2 path.
	testProvider     = "capmox"
	testResourceName = "proxmox-test-image"
	...
	testDeleteResourceName = "proxmox-test-image-delete"
	...
	// Idempotency test: valid qcow2 seeded, imported then re-reconciled.
	testIdempotentResourceName = "proxmox-test-image-idempotent"
	...
	testMissingResourceName = "proxmox-test-image-missing"
	testMissingImageName    = "flatcar-stable-3975.2.0-kube-1.30.4-tooling-1.18.1-missing-gs"

	// Error test: seeded with garbage bytes so the download validation fails.
	testErrorResourceName = "proxmox-test-image-error"
...
var (
	...
	fakeProxmox   *testutil.FakeProxmox
...
func TestIntegration(t *testing.T) {
	...
	RunSpecs(t, "NodeImage Proxmox Integration Suite")
}
...
var _ = BeforeSuite(func() {
	...
	By("starting the in-process Proxmox API simulator")
	fakeProxmox = testutil.StartFakeProxmox()

	By("seeding the in-process S3 bucket with a qcow2 fixture")
	...
	qcow2 := testutil.BuildQCOW2()
	fakeS3, err = testutil.StartFakeS3(imageKey, qcow2)
	Expect(err).NotTo(HaveOccurred())
	...
	Expect(fakeS3.Seed(deleteImageKey, qcow2)).To(Succeed())
	...
	Expect(fakeS3.Seed(idempotentImageKey, qcow2)).To(Succeed())

	// Seeded with bytes that are not a valid qcow2 so the download validation fails.
	...
	Expect(fakeS3.Seed(errorImageKey, []byte("not a valid qcow2"))).To(Succeed())

	// testMissingImageName is intentionally left unseeded.

	// Routes the fake Proxmox server's server-side image download (and the
	// reconciler's HEAD check) to the fake S3 bucket.
	restoreS3 = testutil.RedirectS3Transport(fakeS3)

	By("constructing the real Proxmox provider and S3 client")
	tempDir, err = os.MkdirTemp("", "idop-integration-")
	Expect(err).NotTo(HaveOccurred())
	credentialsPath, locationsPath, err := fakeProxmox.WriteConfig(tempDir)
	Expect(err).NotTo(HaveOccurred())
	...
	reconciler = &imagectrl.NodeImageReconciler{
		Client:    k8sClient,
		S3Client:  s3Client,
		Providers: map[string]provider.Provider{testProvider: proxmoxClient},
	}
})
